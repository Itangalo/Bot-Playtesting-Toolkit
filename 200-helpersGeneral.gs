 ...
 * The returned points are distributed on such a half-circle inside each space, and also turned so the
 * half circle is facing the other space. Only works in two dimensions.
 * 
 * @param {Space} spaceA: The first space to get points within.
 * @param {Space} spaceB: The second space to get points within.
 * @param {Number} numberOfPoints: The number of points to plot. At least 2, defaults to 5.
 * @param {array} coordinates: An array of the coordinate/axis names. Defaults to ['x', 'y'];
 * 
 * @return An array on the form [pointsA, pointsB], where each entry in itself is an array of
 *    points on the form {x: 1, y: 3}, depending on names of coordinates.
 */
function pointHalfCircleDistribution(spaceA, spaceB, numberOfPoints = 5, coordinates = ['x', 'y']) {
