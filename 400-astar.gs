 ...
 */
const aStar = function (graph, heuristic, start, goal, returnPath = true) {
  var paths = [];
  for (var i = 0; i < graph.length; i++) paths[i] = [];
  ...
  while (true) {
    ...
    if (lowestPriorityIndex === -1) {
      ...
      paths[lowestPriorityIndex].push(lowestPriorityIndex);
      if (returnPath) return paths[lowestPriorityIndex];
    ...
    for (var i = 0; i < graph[lowestPriorityIndex].length; i++) {
      if (graph[lowestPriorityIndex][i] !== 0 && !visited[i]) {
        ...
        if (distances[lowestPriorityIndex] + graph[lowestPriorityIndex][i] < distances[i]) {
          ...
          paths[i].push(...paths[lowestPriorityIndex], lowestPriorityIndex)
